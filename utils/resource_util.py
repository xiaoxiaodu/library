...
def get_web_dialogs(version):
    ...
    dirs = []
    if version == '1':
        LIBRARY_WEB_DIALOG_DIRS = settings.LIBRARY_WEB_DIALOG_DIRS
    else:
        LIBRARY_WEB_DIALOG_DIRS = settings.LIBRARY_WEB_DIALOG_DIRS_V2

    for url_path, library_dialog_dir_path in LIBRARY_WEB_DIALOG_DIRS:
        if library_dialog_dir_path.endswith('*'):
            library_dialog_dir_path = library_dialog_dir_path[:-2]
            for f in os.listdir(library_dialog_dir_path):
                if os.path.isdir(os.path.join(library_dialog_dir_path, f)):
                    dirs.append({
                        'module_parent': library_dialog_dir_path,
                        'module': f,
                        'url_path': url_path
                    })
        else:
...
def get_web_views(version):
    ...
    dirs = []
    if version == '1':
        LIBRARY_WEB_VIEW_DIRS = settings.LIBRARY_WEB_VIEW_DIRS
    else:
        LIBRARY_WEB_VIEW_DIRS = settings.LIBRARY_WEB_VIEW_DIRS_V2
    for url_path, library_view_dir_path in LIBRARY_WEB_VIEW_DIRS:
        if library_view_dir_path.endswith('*'):
            library_view_dir_path = library_view_dir_path[:-2]
            for f in os.listdir(library_view_dir_path):
                if os.path.isdir(os.path.join(library_view_dir_path, f)):
                    dirs.append({
                        'module_parent': library_view_dir_path,
                        'module': f,
                        'url_path': url_path
                    })
        else:
...
def get_web_models(version):
    ...
    dirs = []
    if version == '1':
        LIBRARY_WEB_MODEL_DIRS = settings.LIBRARY_WEB_MODEL_DIRS
    else:
        LIBRARY_WEB_MODEL_DIRS = settings.LIBRARY_WEB_MODEL_DIRS_V2
    for url_path, library_view_dir_path in LIBRARY_WEB_MODEL_DIRS:
        if library_view_dir_path.endswith('*'):
            library_view_dir_path = library_view_dir_path[:-2]
            for f in os.listdir(library_view_dir_path):
                if os.path.isdir(os.path.join(library_view_dir_path, f)):
